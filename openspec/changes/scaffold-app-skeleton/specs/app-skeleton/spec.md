...
The app SHALL use Prisma as its ORM against the Compose `db` service, with a committed initial migration that creates at least a placeholder `User` table so the migration path is proven end-to-end.
...
- **AND** the `User` table exists in the `chat` database after boot
