...
- [ ] 2.1 Rework message-row actions into a compact icon-only toolbar that appears on hover and `focus-within`, keeping secondary actions behind the existing overflow menu.
- [ ] 2.2 Increase the visual contrast of the highlighted message row without regressing attachment, reply, or hover states.
- [ ] 2.3 Update the composer/editor chrome so it renders as a single lighter border in idle and focused states instead of stacking dark outer/focus borders.
- [ ] 2.4 Implement the empty-composer `ArrowUp` shortcut so it enters inline edit for the caller's most recent loaded editable message in the active conversation.
- [ ] 2.5 Ensure entering inline edit mode scrolls the edited message into the bottom working area near the composer.
...
- [ ] 4.1 Add or update focused tests for unread badge stability/styling, hover-only message actions, the `ArrowUp` edit shortcut, and room-member copy/invite actions where nearby test coverage makes those regressions meaningful.
