...
- Increase visual contrast for the highlighted message row, briefly blink the source row after reply-jump navigation, and lighten the composer border treatment so the editor keeps a single subtle outline in idle and focus states.
...
- `chat-ui`: tighten conversation-row, message-author, avatar, reply-navigation, composer, message-highlight, message-action, and inline-edit interaction requirements for a denser, more stable chat surface.
...
- Tests should cover unread badge stability, keyboard edit hotkey behavior, reply-jump highlight timing, hover-only message actions, and sending a friend request from a room member row.
