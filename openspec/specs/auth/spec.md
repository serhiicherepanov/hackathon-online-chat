...
Defines the authentication capability for the hackathon online chat: account registration, sign-in by email or username, persistent per-browser sessions backed by a `Session` table, sign-out scoped to the current browser, current-user resolution for the app shell, and username immutability. Sessions are persistent (no forced logout from inactivity), independent across browsers/tabs, and rely on argon2id-hashed passwords.
