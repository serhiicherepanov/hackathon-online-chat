...
The system SHALL stream attachment bytes at `GET /api/files/:id` only when the caller is, at request time, a member of the room conversation or a participant of the DM conversation that the attachment's message belongs to.
...
- **WHEN** an authenticated user who is neither a `RoomMember` for the attachment's room nor a `DmParticipant` for its DM calls `GET /api/files/:id`
- **THEN** the server responds `403 Forbidden`
