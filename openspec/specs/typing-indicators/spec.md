...
Defines typing indicators for room and DM conversations: throttled, ephemeral `typing` events published while a participant is actively composing non-empty text, local 3-second expiration without explicit stop events, and suppression for empty composers and frozen direct messages.
