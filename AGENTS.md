...
- **Environment**: document required vars (e.g. `DATABASE_URL`, Centrifugo HTTP API / WebSocket URLs, tokens, `VAPID_*` keys for Web Push) in `.env.example` when adding features—never commit secrets. For desktop notifications, regenerate the VAPID keypair per environment via `docker compose exec app pnpm tsx scripts/gen-vapid.ts` and pass `NEXT_PUBLIC_VAPID_PUBLIC_KEY` as a build ARG so it is inlined into the client bundle.
